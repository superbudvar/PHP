...
class PaymentApi
{
     ...
     * @param  \FirstData\FirstApi\Client\Model\AuthenticationVerificationRequest $authenticationVerificationRequest Accepted request types: Secure3dAuthenticationVerificationRequest and UnionPayAuthenticationVerificationRequest. (required)
     ...
     */
    public function finalizeSecureTransaction($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature = null, $region = null)
    {
        list($response) = $this->finalizeSecureTransactionWithHttpInfo($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature, $region);
        return $response;
    }
     ...
     */
    public function finalizeSecureTransactionWithHttpInfo($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature = null, $region = null)
    {
        $request = $this->finalizeSecureTransactionRequest($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature, $region);
     ...
     */
    public function finalizeSecureTransactionAsync($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature = null, $region = null)
    {
        return $this->finalizeSecureTransactionAsyncWithHttpInfo($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature, $region)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function finalizeSecureTransactionAsyncWithHttpInfo($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature = null, $region = null)
    {
        $returnType = '\FirstData\FirstApi\Client\Model\TransactionResponse';
        $request = $this->finalizeSecureTransactionRequest($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature, $region);
     ...
     */
    protected function finalizeSecureTransactionRequest($contentType, $clientRequestId, $apiKey, $timestamp, $transactionId, $authenticationVerificationRequest, $messageSignature = null, $region = null)
    {
        ...
        if ($authenticationVerificationRequest === null || (is_array($authenticationVerificationRequest) && count($authenticationVerificationRequest) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $authenticationVerificationRequest when calling finalizeSecureTransaction'
            );
        }
        ...
        // body params
        $_tempBody = null;
        if (isset($authenticationVerificationRequest)) {
            $_tempBody = $authenticationVerificationRequest;
        }
     ...
     * @param  int $timestamp Epoch timestamp in milliseconds in the request from a client system. Used for Message Signature generation and time limit (5 mins). (required)
     * @param  string $transactionId Gateway transaction identifier as returned in the parameter ipgTransactionId. (required)
     * @param  \FirstData\FirstApi\Client\Model\SecondaryTransaction $secondaryTransaction Accepted request types: PostAuthTransaction, VoidTransaction, and ReturnTransaction. (required)
     * @param  string $messageSignature Used to ensure the request has not been tampered with during transmission. The Message-Signature is the Base64 encoded HMAC hash (SHA256 algorithm with the API Secret as the key.) For more information, refer to the supporting documentation on the Developer Portal. (optional)
     * @param  string $region Indicates the region where the client wants the transaction to be processed. This will override the default processing region identified for the client. Available options are argentina, brazil, germany, india and northamerica. Region specific store setup and APIGEE boarding is required in order to use an alternate region for processing. (optional)
